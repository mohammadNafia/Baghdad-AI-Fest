...
import { Search, Clock, CheckCircle2, XCircle, Home, Download, Calendar, MapPin, ArrowLeft } from 'lucide-react';
...
const CheckTicket: React.FC = () => {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      // Search in localStorage (replace with Supabase query when ready)
      const attendees = JSON.parse(localStorage.getItem('attendees') || '[]');
      const found = attendees.find((a: AttendeeFormData) => 
      ...
      if (found) {
        ...
        if (found.status === 'approved') {
          await generateQRCode(found.email);
        }
      } else {
        setNotFound(true);
      }
          ...
          {searchResult && searchResult.status === 'pending' && (
            <div className={`p-6 rounded-2xl border text-center ${
              ...
              <p className={`text-sm mb-4 ${
                ...
                {lang === 'ar' 
                  ? 'نقوم حالياً بمراجعة طلبك. سنتواصل معك قريباً.'
                  : 'Your application is still being reviewed. We will notify you soon.'}
              </p>
              <p className={`text-xs ${
          ...
          {searchResult && searchResult.status === 'rejected' && (
            <div className={`p-6 rounded-2xl border text-center ${
              ...
              <h3 className={`font-semibold mb-2 ${
                ...
                {lang === 'ar' ? 'لم تتم الموافقة على الطلب' : 'Application Not Approved'}
              </h3>
              <p className={`text-sm ${
                theme === 'light' ? 'text-red-600' : 'text-red-400'
              }`}>
                {lang === 'ar' 
                  ? 'نأسف، لم نتمكن من الموافقة على طلبك هذه المرة.'
                  : 'Unfortunately, we were unable to approve your application this time.'}
              </p>
            </div>
          )}
